columns_to_remove <- c(
  ...
  "upload_2.2. Secondary Phone Number:",
  "upload_2.2. Secondary Phone Number",
  "upload_2.2. Secondary Phone Number:_a",
  "upload_2.2. Secondary Phone Number:_b",
  "upload_2.2. Secondary Phone Number_a",
  "upload_2.2. Secondary Phone Number_b",
  "upload_secondary_phone_number",
  "upload_secondary_phone_number_a",
  "upload_secondary_phone_number_b",
  "upload_QA_CODE_SN",
  "upload_QA_CODE_SN_a",
  "upload_QA_CODE_SN_b",
  "upload_Batch_ID",
...
drop_export_columns <- function(df) {
  ...
  patterns_to_drop <- c(
    ...
    "^upload_2\\.2\\.[ _]Secondary[ _]Phone[ _]Number:?(_[ab])?$",
    "^upload_secondary_phone_number(_[ab])?$",
...
reorder_upload_master_columns <- function(df) {
  ...
  if ("master_dist_type" %in% names(df)) {
    names(df)[names(df) == "master_dist_type"] <- "Assistance Type"
  }
  cols <- names(df)
  upload_cols <- grep("^upload_", cols, value = TRUE)
  master_cols <- unique(c(
    grep("^master_", cols, value = TRUE),
    intersect(c("Last Receipt Date", "master_Last Receipt Date", "Assistance Type", "master_Assistance Type"), cols)
  ))
  ...
  pair_defs <- list(
    ...
    # 13. MPCA Distribution Date / Last Receipt Date & Assistance Type
    list(
      u_fn = function(x) grepl("dist.*date|mpca.*date|receipt.*date", x, ignore.case = TRUE),
      m_fn = function(x) grepl("^(master_)?(dist_date_calc_new|Last[ _]Receipt[ _]Date|dist_type|Assistance[ _]Type)$", x, ignore.case = TRUE)
    )
  )
...
build_audit_manifest_sheet <- function(result, audit_meta = list(), triage_decisions = NULL) {
  upload_hash <- audit_meta$upload_sha256 %||% result$upload_sha256 %||% "Not Recorded"
  master_hash <- audit_meta$master_sha256 %||% result$master_sha256 %||% "Not Recorded"
  manifest_id <- audit_meta$manifest_id %||% result$manifest_id %||% paste0("CCY-AUDIT-", format(Sys.time(), "%Y%m%d%H%M%S-"), sample(1000:9999, 1))

  # Summary metrics extraction from result
  upload_n <- NA_integer_
  master_n <- NA_integer_
  if (!is.null(result$summary) && is.data.frame(result$summary) && "metric" %in% names(result$summary)) {
    u_idx <- which(result$summary$metric == "Total Records Examined")
    if (length(u_idx) == 1) upload_n <- suppressWarnings(as.integer(result$summary$value[u_idx]))
    m_idx <- which(result$summary$metric == "Master Record Count")
    if (length(m_idx) == 1) master_n <- suppressWarnings(as.integer(result$summary$value[m_idx]))
  }
  if (is.na(upload_n)) {
    upload_n <- as.integer(audit_meta$record_count %||% 0L)
  }

  sl_exact_df <- result$same_list_exact %||% data.frame()
  sl_fuzzy_df <- result$same_list_fuzzy %||% result$same_list_medium %||% data.frame()
  lm_exact_df <- result$list_vs_master_exact %||% data.frame()
  lm_fuzzy_df <- result$list_vs_master_fuzzy %||% result$list_vs_master_medium %||% data.frame()

  n_sl_exact <- nrow(sl_exact_df)
  n_sl_fuzzy <- nrow(sl_fuzzy_df)
  n_lm_exact <- nrow(lm_exact_df)
  n_lm_fuzzy <- nrow(lm_fuzzy_df)

  n_high <- nrow(result$same_list_high %||% result$same_list_exact %||% data.frame()) + nrow(result$list_vs_master_high %||% result$list_vs_master_exact %||% data.frame())
  n_med <- nrow(result$same_list_medium %||% result$same_list_fuzzy %||% data.frame()) + nrow(result$list_vs_master_medium %||% result$list_vs_master_fuzzy %||% data.frame())
  total_dups <- n_high + n_med

  pct_str <- function(num, den) {
    if (is.na(den) || den <= 0) return(as.character(num))
    paste0(format(num, big.mark = ","), " (", sprintf("%.2f%%", 100 * num / den), ")")
  }

  dedup_rate_str <- if (!is.na(upload_n) && upload_n > 0) sprintf("%.2f%%", 100 * total_dups / upload_n) else "N/A"

  # Triage counts if available
  n_triaged <- 0L
  if (!is.null(triage_decisions) && is.list(triage_decisions)) {
    n_triaged <- sum(vapply(triage_decisions, function(x) !is.null(x$status) && x$status != "Unreviewed", logical(1)))
  }
  ...
  data.frame(
    Property = c(
      "Audit Manifest ID",
      "Verification Engine",
      "Upload Dataset SHA-256 Digest",
      "Master Snapshot SHA-256 Digest",
      "Export Timestamp (UTC)",
      "Executed By (User)",
      "Security Role",
      "Partner Organization",
      "Upload Records Examined",
      "Master Records Evaluated",
      "High Confidence Threshold",
      "Medium Confidence Threshold",
      "Max Evaluated Candidates",
      "MPCA Assistance Recency Filter",
      "Total High Confidence Duplicates",
      "Total Medium Review Duplicates",
      "Same List Exact Matches",
      "Same List Fuzzy Matches",
      "List vs ActivityInfo Exact Matches",
      "List vs ActivityInfo Fuzzy Matches",
      "Overall Deduplication Rate",
      "Triage Decisions Completed",
      "Integrity & Compliance Statement"
    ),
    Value = c(
      manifest_id,
      "SHA-256 FIPS-Compliant Hash Digest",
      upload_hash,
      master_hash,
      format(Sys.time(), "%Y-%m-%d %H:%M:%S UTC", tz = "UTC"),
      as.character(audit_meta$user_email %||% "Unknown User"),
      as.character(audit_meta$user_role %||% "Unknown Role"),
      as.character(audit_meta$partner_name %||% "CCY Consortium"),
      if (!is.na(upload_n)) format(upload_n, big.mark = ",") else "Not Recorded",
      if (!is.na(master_n)) format(master_n, big.mark = ",") else "Not Recorded",
      paste0(result$fuzzy_high_threshold %||% (if (exists("config") && !is.null(config$thresholds$high)) config$thresholds$high else 90), "%"),
      paste0(result$fuzzy_medium_threshold %||% (if (exists("config") && !is.null(config$thresholds$medium)) config$thresholds$medium else 75), "%"),
      as.character(result$max_candidates %||% (if (exists("config") && !is.null(config$max_candidates)) config$max_candidates else 500)),
      if (isTRUE(result$filter_recent_mpca)) paste0("Active (< ", result$mpca_window_months %||% 6, " months)") else "Disabled (All Historical Records)",
      as.character(n_high),
      as.character(n_med),
      pct_str(n_sl_exact, upload_n),
      pct_str(n_sl_fuzzy, upload_n),
      pct_str(n_lm_exact, upload_n),
      pct_str(n_lm_fuzzy, upload_n),
      dedup_rate_str,
      if (n_triaged > 0) paste0(n_triaged, " / ", total_dups) else "None Recorded (Unreviewed)",
      "Generated by CCY Deduplication Platform. Cryptographic SHA-256 digests verify data authenticity at time of matching. Any manual alterations outside the system will invalidate this audit dossier."
    ),
    stringsAsFactors = FALSE
  )
}

write_dedup_workbook <- function(result, file, triage_decisions = NULL, audit_meta = list()) {
  wb <- openxlsx::createWorkbook()

  info_df <- normalize_export_table(result$info)
  manifest_df <- build_audit_manifest_sheet(result, audit_meta, triage_decisions = triage_decisions)

  write_formatted_sheet(wb, "Info", info_df, highlight_score = FALSE, tab_color = "#0F766E")
  write_formatted_sheet(wb, "Audit_Manifest", manifest_df, highlight_score = FALSE, tab_color = "#334155")
