...
write_dedup_workbook <- function(result, file) {
  ...
  get_sheet <- function(keys) {
    for (k in keys) {
      if (!is.null(result[[k]])) return(result[[k]])
    }
    return(data.frame())
  }

  write_formatted_sheet(wb, "Same List (High Confidence)", get_sheet(c("same_list_high", "same_list_exact")), highlight_score = TRUE)
  write_formatted_sheet(wb, "Same List (Medium Confidence)", get_sheet(c("same_list_medium", "same_list_fuzzy")), highlight_score = TRUE)
  write_formatted_sheet(wb, "List vs ActivityInfo (High)", get_sheet(c("list_vs_master_high", "list_vs_master_exact")), highlight_score = TRUE)
  write_formatted_sheet(wb, "List vs ActivityInfo (Medium)", get_sheet(c("list_vs_master_medium", "list_vs_master_fuzzy")), highlight_score = TRUE)
